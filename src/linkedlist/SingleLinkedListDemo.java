...
//        singleLinkedList.list();
        ...
        //测试逆序打印单链表
//        System.out.println("测试逆序打印单链表,没有改变 链表本身的结构");
...
//        System.out.println("res=" + res);
    ...
    public static void merge(SingleLinkedList list1, SingleLinkedList list2) {
        SingleLinkedList mergeLinkList = new SingleLinkedList();

        //获得链表1第一个值
        HeroNode head = mergeLinkList.getHead();

        HeroNode temp1 = list1.getHead().next;
        System.out.println("temp1 == > " + temp1.no);
        //获得链表2第一个值
        HeroNode temp2 = list2.getHead().next;
        System.out.println("temp2 == > " + temp2.no);
    ...
    public static void reverseList(HeroNode head) {
        ...
        //遍历原来的链表,就将其取出，并放在链表的reverseHead 的最前端
        while (cur != null) {
...
//定义SingleLinkedList 管理我们的英雄
 class SingleLinkedList {
    ...
    public void addByOrder(HeroNode heroNode) {
        //因为头节点不能动，因此我们仍然啊通过一个辅助指针（变量）来帮助找到添加的位置
        //因为单链表，因为我们找的temp 是位于 添加位置的前一个节点，否则插入插入不了
        HeroNode temp = head;
        ...
        while (true) {
            ...
            if (temp.next.no > heroNode.no) {//位置找到了，就要在temp的后面插入
                break;
            } else if (temp.next.no == heroNode.no) {//说明希望添加的herNode编号已然存在
                flag = true;//说明编号存在
                break;
            }
    ...
    //2. 说明我们在比较时，是temp.next.no 和 需要删除的节点No进行比较

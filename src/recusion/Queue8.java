...
public class Queue8 {
    ...
    //定义一个max表示共有多少个皇后
    int max = 8;
    //定义数据组array,保存皇后放置位置的结果，比如arr[8]={0,4,7,5,2,6,1,3}
    ...
    //编写一个方法，放置第n个皇后
    //特别注意:check 是每一次递归时，进入到check中都有，for(int i=0;i<max;i++),因此有回溯
    ...
    private boolean judge(int n) {
        ...
        for (int i = 0; i < n; i++) {
            //说明
            //1.array[i]==array[n]||Math.abs(n-i)==Math.abs(array[n]-array[i]) 表示判断 第n个皇后是否和前面的n-1个皇后在同一列
            //2.Math.abs(n-i)==Math.abs(array[n]-array[i]) 表示判断第n个皇后是否和第i个皇后是否在同一斜线
            //假设 n=1 放在第2列1 n=1 array[1]=1
            //Math.abs(1-0) == 1 Math.abs(array[n] - array[i])== 1
            //3.判断同一行，没有必要，n 每次都在递增
